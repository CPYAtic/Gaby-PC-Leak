...
void Console::PrintPackets(const char* Info, char* buff, int len, int pName)
{
	...
	Print(color, 1, "[%s]", timestamp.str());
	Print(color, 0, "[SIZE: %d]",len);

	for (int i = 0; i < len; i++)
		Print(color, 0, " %02X", static_cast<byte>(buff[i]));

	Print(color, 0, "\n");
	*/
	if (strcmp(Info,"RECV") == 0)
		printf("[PNAME: %d] ", pName);

	printf("%s [SIZE: %d]", Info, len);
