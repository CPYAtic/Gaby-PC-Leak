...
void Server::ListenAsync()
{
	...
	while (true)
	{
		...
		if (newsock != INVALID_SOCKET)
		{
			...
			Peer* peer = new Peer(newsock, ipstr, std::string(std::string("feel-dope[") + std::to_string(i++) + std::string("]")).c_str());
			if (peer == nullptr)
			{
				Console::PrintErr(0, "Failed to create a peer instance.\n");
				Console::Print(Console::Yellow, 0, "Retrying...");

				peer = new Peer(newsock, ipstr, std::string(std::string("feel-dope[") + std::to_string(i) + std::string("]")).c_str());
				if (peer == nullptr)
				{
