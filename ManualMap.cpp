...
bool ManualMapFromMemory(HANDLE hProc, char * szData, DWORD FileSize)
{
	BYTE *					pSrcData = nullptr;
	IMAGE_NT_HEADERS *		pOldNtHeader = nullptr;
	IMAGE_OPTIONAL_HEADER * pOldOptHeader = nullptr;
	...
	if (reinterpret_cast<IMAGE_DOS_HEADER*>(szData)->e_magic != 0x5A4D) //"MZ"
	{
		//printf("Invalid file\n");
		return false;
	}

	pOldNtHeader = reinterpret_cast<IMAGE_NT_HEADERS*>(szData + reinterpret_cast<IMAGE_DOS_HEADER*>(szData)->e_lfanew);
	pOldOptHeader = &pOldNtHeader->OptionalHeader;
	pOldFileHeader = &pOldNtHeader->FileHeader;
	...
	for (UINT i = 0; i != pOldFileHeader->NumberOfSections; ++i, ++pSectionHeader)
	{
		if (pSectionHeader->SizeOfRawData)
		{
			if (!WriteProcessMemory(hProc, pTargetBase + pSectionHeader->VirtualAddress, szData + pSectionHeader->PointerToRawData, pSectionHeader->SizeOfRawData, nullptr))
			{
				//printf("Can't map sections: 0x%x\n", GetLastError());
				VirtualFreeEx(hProc, pTargetBase, 0, MEM_RELEASE);
				return false;
	...
	WriteProcessMemory(hProc, pTargetBase, szData, 0x1000, nullptr);
